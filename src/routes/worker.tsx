...
import { setOrderDelivered } from "@/lib/scores.functions";
...
function WorkerPage() {
  ...
  const deliver = useServerFn(setOrderDelivered);
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-6 md:py-8">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2"><button onClick={() => navigate({ to: "/exams" })} className="rounded-xl border-2 border-border bg-card px-4 py-2 text-sm font-extrabold hover:bg-secondary">📝 Exams</button><button onClick={() => navigate({ to: "/leaderboard" })} className="rounded-xl border-2 border-border bg-card px-4 py-2 text-sm font-extrabold hover:bg-secondary">🏆 Leaderboard</button><UpdateButton /><button onClick={() => { clearSession(); navigate({ to: "/" }); }} className="rounded-xl border-2 border-border bg-card px-4 py-2 text-sm font-bold hover:bg-secondary">Log out</button></div>
      </header>
      ...
      <section className="mb-6"><div className="mb-3 flex items-end justify-between"><div><p className="text-xs font-extrabold uppercase tracking-wider text-primary">Your toolbox</p><h2 className="text-2xl font-extrabold">Jump into something fun</h2></div><span className="rounded-full bg-secondary px-3 py-1 text-xs font-extrabold">{features.length} spaces</span></div><div className="grid grid-cols-2 gap-3 md:grid-cols-3">{features.map((feature, i) => <button key={feature} onClick={() => navigate({ to: "/space/$spaceKey", params: { spaceKey: `space-${i + 1}` } })} className="group rounded-2xl border-2 border-border bg-card p-4 text-left font-extrabold shadow-sm transition hover:-translate-y-1 hover:border-primary hover:shadow-lg"><div className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-secondary text-xl">{["✨","🚀","🎯","🧩","🔥","💡"][i % 6]}</div><span>{feature}</span><p className="mt-1 text-xs font-semibold text-muted-foreground">Open workspace →</p></button>)}</div></section>
