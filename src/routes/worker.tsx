...
function WorkerPage() {
  ...
  const orderMut = useMutation({ mutationFn: () => create({ data: { code: session!.code, companyId, customerName: form.customerName, orderDetails: form.orderDetails, price: Number(form.price || 0), pricePaid: Number(form.pricePaid || 0) } }), onSuccess: () => { toast.success("Order added 🎉 +20 XP"); setForm({ customerName: "", orderDetails: "", price: "", pricePaid: "" }); qc.invalidateQueries({ queryKey: ["worker", session?.code, companyId] }); }, onError: () => toast.error("Please fill in the order details and amounts.") });
  const editMut = useMutation({ mutationFn: (id: string) => edit({ data: { code: session!.code, companyId, id, customerName: editForm.customerName, orderDetails: editForm.orderDetails, price: Number(editForm.price || 0), pricePaid: Number(editForm.pricePaid || 0) } }), onSuccess: () => { toast.success("Order updated ✏️"); setEditingId(null); qc.invalidateQueries({ queryKey: ["worker", session?.code, companyId] }); }, onError: () => toast.error("Could not update this order.") });
