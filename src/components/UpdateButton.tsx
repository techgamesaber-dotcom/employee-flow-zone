  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onMouseDown={(e) => { if (e.target === e.currentTarget) setOpen(false); }}>
      <section className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-3xl border-2 border-border bg-card p-5 shadow-2xl">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2">{grouped.map((space, i) => <div key={space.key} className="rounded-2xl border-2 border-border bg-background/60 p-4"><div className="flex items-center justify-between gap-2"><div><span className="text-xl">{["✨","🚀","🎯","🧩","🔥","💡"][i]}</span><h3 className="mt-1 font-extrabold">{space.name}</h3></div><span className="rounded-full bg-secondary px-3 py-1 text-xs font-extrabold">{space.items.length} items</span></div><div className="mt-3 space-y-2">{space.items.map((item) => <div key={item.id} className="rounded-xl border border-border bg-card p-3"><div className="flex items-start justify-between gap-2"><div><p className="font-extrabold">{item.title}</p>{item.description && <p className="mt-1 text-xs font-semibold text-muted-foreground">{item.description}</p>}</div>{session?.isAdmin && <button onClick={() => remove.mutate(item.id)} disabled={remove.isPending} className="rounded-lg bg-berry/10 px-2 py-1 text-xs font-extrabold text-berry-foreground">− Remove</button>}</div></div>)}{!space.items.length && <p className="text-sm font-semibold text-muted-foreground">Nothing here yet.</p>}</div></div>)}</div>
      </section>
    </div>}
  </>;
}
