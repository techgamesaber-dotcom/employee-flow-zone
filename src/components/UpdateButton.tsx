...
export function UpdateButton() {
  ...
  const add = useMutation({
    ...
    onSuccess: () => { setTitle(""); setDescription(""); toast.success("Workspace item added ✨"); qc.invalidateQueries({ queryKey: ["workspace", session?.code, companyId] }); },
  ...
  const grouped = useMemo(() => spaceNames.map((name, i) => ({ key: `space-${i + 1}`, name, items: (items.data ?? []).filter((x) => x.space_key === `space-${i + 1}`) })), [spaceNames, items.data]);

  // Make the six existing toolbox cards open the real workspace instead of saying "queued".
  useEffect(() => {
  ...
  // Pressing Update app reloads the same URL with a cache-busting version marker.
  // After the newest deployed build loads, the workspace opens automatically once.
  useEffect(() => {
  ...
  async function handleUpdate() {
    ...
    } catch { /* reload below still works */ }
    const url = new URL(window.location.href);
  ...
  return <>
    <div className="flex gap-2">
      <button onClick={() => setOpen(true)} className="rounded-xl border-2 border-border bg-card px-4 py-2 text-sm font-extrabold shadow-sm transition hover:-translate-y-0.5 hover:bg-secondary">✨ Spaces</button>
    ...
    {open && <div data-workspace-modal className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onMouseDown={(e) => { if (e.target === e.currentTarget) setOpen(false); }}>
      <section className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-3xl border-2 border-border bg-card p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-wider text-primary">{company?.emoji} {company?.name}</p><h2 className="text-2xl font-extrabold">Your six workspaces</h2><p className="text-sm font-semibold text-muted-foreground">Choose a workspace to view its library.</p></div><button onClick={() => setOpen(false)} className="rounded-xl border-2 border-border px-3 py-2 font-bold">✕</button></div>
        {session?.isAdmin && <form onSubmit={(e) => { e.preventDefault(); if (!title.trim()) return; add.mutate(); }} className="mt-5 rounded-2xl border-2 border-primary/20 bg-secondary p-4"><p className="font-extrabold">Admin: add to a workspace</p><div className="mt-3 grid gap-3 md:grid-cols-4"><select value={spaceKey} onChange={(e) => setSpaceKey(e.target.value)} className="rounded-xl border-2 border-border bg-card px-3 py-3 font-semibold">{spaceNames.map((name, i) => <option key={name} value={`space-${i + 1}`}>{name}</option>)}</select><input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Item title" className="rounded-xl border-2 border-border bg-card px-3 py-3 font-semibold" /><input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Short description" className="rounded-xl border-2 border-border bg-card px-3 py-3 font-semibold" /><button disabled={add.isPending} className="rounded-xl bg-primary px-4 py-3 font-extrabold text-primary-foreground">{add.isPending ? "Adding…" : "＋ Add"}</button></div></form>}
        <div className="mt-5 grid gap-3 md:grid-cols-2">{grouped.map((space, i) => <div key={space.key} className={`rounded-2xl border-2 p-4 ${space.key === spaceKey ? "border-primary bg-primary/5" : "border-border bg-background/60"}`}><button type="button" data-workspace-control onClick={() => setSpaceKey(space.key)} className="w-full text-left"><div className="flex items-center justify-between gap-2"><div><span className="text-xl">{icons[i]}</span><h3 className="mt-1 font-extrabold">{space.name}</h3></div><span className="rounded-full bg-secondary px-3 py-1 text-xs font-extrabold">{space.items.length} items</span></div></button><div className="mt-3 space-y-2">{space.items.map((item) => <div key={item.id} className="rounded-xl border border-border bg-card p-3"><div className="flex items-start justify-between gap-2"><div><p className="font-extrabold">{item.title}</p>{item.description && <p className="mt-1 text-xs font-semibold text-muted-foreground">{item.description}</p>}</div>{session?.isAdmin && <button type="button" data-workspace-control onClick={() => remove.mutate(item.id)} disabled={remove.isPending} className="rounded-lg bg-berry/10 px-2 py-1 text-xs font-extrabold text-berry-foreground">− Remove</button>}</div></div>)}{!space.items.length && <p className="text-sm font-semibold text-muted-foreground">Nothing here yet.</p>}</div></div>)}</div>
      </section>
    </div>}
  </>;
}
