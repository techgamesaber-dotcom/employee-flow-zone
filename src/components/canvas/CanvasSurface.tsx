...
export function ElementView({ el }: { el: CanvasElement }) {
  ...
  switch (el.type) {
    case "heading":
      return (
        <h2 className="font-extrabold leading-tight" style={{ fontSize: c.size ?? 34, color: c.color || undefined, textAlign: c.align ?? "left" }}>
          {c.text || "Your heading"}
        </h2>
      );
    case "text":
      return (
        <p className="whitespace-pre-wrap font-semibold leading-relaxed" style={{ fontSize: c.size ?? 16, color: c.color || undefined, textAlign: c.align ?? "left" }}>
          {c.text || "Write something here…"}
        </p>
      );
    case "image":
    ...
    case "youtube": {
      ...
      return embed ? (
        <iframe className="h-full w-full rounded-xl" src={embed} title="YouTube video" allow="accelerometer; clipboard-write; encrypted-media; picture-in-picture" allowFullScreen />
      ) : (
        <Empty label="▶️ Paste a YouTube link" />
      );
    }
    case "link":
      return (
        <a href={c.url || "#"} target="_blank" rel="noreferrer" className="flex h-full w-full items-center gap-3 rounded-xl border-2 border-border bg-card px-4 font-extrabold hover:border-primary">
          <span className="text-xl">🔗</span>
          <span className="truncate">{c.label || c.url || "Add a link"}</span>
        </a>
      );
    case "file":
      return (
        <a href={c.url || "#"} target="_blank" rel="noreferrer" className="flex h-full w-full items-center gap-3 rounded-xl border-2 border-border bg-card px-4 font-extrabold hover:border-primary">
          <span className="text-xl">📄</span>
          <span className="truncate">{c.name || "Upload a document"}</span>
        </a>
      );
    case "gallery": {
      ...
      return (
        <div className="grid h-full w-full grid-cols-3 gap-2 overflow-hidden">
          {urls.map((u, i) => (
            <img key={i} src={u} alt={`Gallery ${i + 1}`} className="h-full w-full rounded-lg object-cover" />
          ))}
        </div>
      );
    }
    ...
    case "table": {
      ...
      return (
        <div className="h-full w-full overflow-auto rounded-xl border-2 border-border bg-card">
          <table className="w-full text-left text-sm">
            <tbody>
              {rows.map((row, ri) => (
                <tr key={ri} className={ri === 0 ? "bg-secondary" : "border-t border-border"}>
                  {row.map((cell, ci) => (
                    <td key={ci} className={`px-3 py-2 ${ri === 0 ? "font-extrabold" : "font-semibold"}`}>{cell}</td>
                  ))}
                </tr>
              ))}
            </tbody>
...
export function CanvasSurface({
  ...
  return (
    <div className="overflow-auto rounded-3xl border-2 border-border bg-background p-4 shadow-inner">
      <div
        ref={surface}
        onPointerMove={move}
        onPointerUp={end}
        ...
        className="relative mx-auto rounded-2xl bg-card"
        style={{
          width: CANVAS_W,
          height: CANVAS_H,
      ...
      >
        {elements.map((el) => {
          ...
          return (
            <div
              ...
              onPointerMove={move}
              onPointerUp={end}
              className={`absolute ${editable ? "cursor-move" : ""} ${selected ? "ring-4 ring-primary/60" : editable ? "hover:ring-2 hover:ring-primary/30" : ""} rounded-xl`}
              style={{ left: el.x, top: el.y, width: el.w, height: el.h, zIndex: el.z }}
            >
              <div className="pointer-events-none h-full w-full overflow-hidden" style={{ pointerEvents: editable ? "none" : "auto" }}>
                <ElementView el={el} />
              </div>
              {selected && (
                <div
                  ...
                  className="absolute -bottom-2 -right-2 h-5 w-5 cursor-nwse-resize rounded-full border-2 border-card bg-primary"
                />
              )}
            </div>
          );
        ...
        {!elements.length && (
          <p className="absolute left-1/2 top-24 -translate-x-1/2 text-sm font-extrabold text-muted-foreground">
            {editable ? "This space is empty — use ＋ Add to place anything you like." : "Nothing published in this space yet."}
          </p>
        )}
      </div>
    </div>
